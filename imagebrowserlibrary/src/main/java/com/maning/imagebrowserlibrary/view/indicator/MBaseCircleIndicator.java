...
class MBaseCircleIndicator extends LinearLayout {
    ...
    public MBaseCircleIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public MBaseCircleIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private Config handleTypedArray(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.MBaseCircleIndicator);
        config.width =
                typedArray.getDimensionPixelSize(R.styleable.MBaseCircleIndicator_ci_width, -1);
        config.height =
                typedArray.getDimensionPixelSize(R.styleable.MBaseCircleIndicator_ci_height, -1);
        config.margin =
                typedArray.getDimensionPixelSize(R.styleable.MBaseCircleIndicator_ci_margin, -1);
        config.animatorResId = typedArray.getResourceId(R.styleable.MBaseCircleIndicator_ci_animator,
                R.animator.mn_browser_indicator_scale);
        config.animatorReverseResId =
                typedArray.getResourceId(R.styleable.MBaseCircleIndicator_ci_animator_reverse, 0);
        config.backgroundResId =
                typedArray.getResourceId(R.styleable.MBaseCircleIndicator_ci_drawable,
                        R.drawable.mn_browser_indicator_bg_selected);
        config.unselectedBackgroundId =
                typedArray.getResourceId(R.styleable.MBaseCircleIndicator_ci_drawable_unselected,
                        config.backgroundResId);
        config.orientation = typedArray.getInt(R.styleable.MBaseCircleIndicator_ci_orientation, -1);
        config.gravity = typedArray.getInt(R.styleable.MBaseCircleIndicator_ci_gravity, -1);
        typedArray.recycle();
    ...
    public void initialize(Config config) {
        ...
        mIndicatorBackgroundResId =
                (config.backgroundResId == 0) ? R.drawable.mn_browser_indicator_bg_selected : config.backgroundResId;
